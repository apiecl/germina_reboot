...
$(document).ready(function() {
    ...
    typebuttons.each(function( index, element ) {
      ...
      var countelements = $('div.tax-item-medium[data-type="' + filter + '"]').length;
    ...
    videocontrol.on('click', function() {
        $(this)
            .empty()
            .append('<video class="video-germina video-js" src="' + germina.video_url + '">Se necesita un navegador compatible con HTML5 para ver este video.</video>');
    });
    ...
    portafolio.on('cycle-update-view', function(event, optionHash, slideOptionHash, currentslideEl) {
	...
	(function($, viewport){
        ...
        if(viewport.is('xs')) {
            ...
            $('a[data-function="toggle-nav"]').on('click', function() {
                var el = $( '[data-id="' + $(this).attr('data-target') + '"]' );
        ...
        if(viewport.is('>=sm')) {
            ...
            var $pubgrid = $('.publicaciones-wrapper').imagesLoaded( function() {
                $pubgrid.masonry({
                    itemSelector: '.publicacion-item-medium'
                });
            });

            var $attachedgrid = $('div.attached-to-post.Miniaturas').imagesLoaded( function() {
